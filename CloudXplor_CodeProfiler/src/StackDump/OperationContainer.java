...
public class OperationContainer {
	...
	public void getAllMethodsAllThreads(BufferedWriter bw, ArrayList <Long> ids) throws IOException {
		if (bw == null) {
			...
			bw = new BufferedWriter(new FileWriter (new File("StackDump-" + formatter.format(date) + ".txt"), true));
		}
	...
	public void getAll() throws IOException {
		SimpleDateFormat formatter= new SimpleDateFormat("yyyy-MM-dd_HH-mm-ss");
		Date date = new Date(System.currentTimeMillis());
		BufferedWriter bw = new BufferedWriter(new FileWriter (new File("StackDump-" + formatter.format(date) + ".txt"), true));
		...
		class AMAT implements Runnable {
			...
			public void run() {
				try {
					new OperationContainer().getAllMethodsAllThreads(bw, this.arr);
				} catch (IOException e) {
		...
		class AMOT implements Runnable {
			...
			public void run() {
				try {
					new OperationContainer().getAllMethodsOneThread(Thread.currentThread(), bw);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		class MCAT implements Runnable {
			...
			public void run() {
				try {
					new OperationContainer().MethodCountAllThread(bw);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
		...
		class MCOT implements Runnable {
			...
			public void run() {
				try {
					new OperationContainer().MethodCountOneThread(Thread.currentThread(), "run", bw);
				} catch (IOException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
