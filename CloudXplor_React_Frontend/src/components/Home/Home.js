...
class Home extends Component {
  ...
  componentDidMount() {
    this.updateMemData()
    setInterval(this.updateMemData, 1000);

    this.updateDiskData()
    setInterval(this.updateDiskData, 1000);
    
    this.updateCpuData()
    setInterval(this.updateCpuData, 1000);

    this.fetchIoData()
    setInterval(this.fetchIoData, 1000);
  }
  ...
  ioDeviceClicked (index) {
    ...
    let temp = parseInt(ioData[index].write);
    temp = temp / 1000000;
    temp = temp.toFixed(2);
    
    this.setState( {
        tps: parseInt(ioData[index].tps),
        readSpeed: parseInt(ioData[index].readSpeed),
        writeSpeed: parseInt(ioData[index].writeSpeed),
        read: parseInt(ioData[index].read),
        write: temp,
        device_id : index,
    });
  }
