...
class Predictions extends Component {
  ...
  async componentDidMount() {
    this.fetchTotalAnomalies();
    setInterval(this.fetchTotalAnomalies, 30000);

    this.fetchAnomalyData();
    setInterval(this.fetchAnomalyData, 30000);
  }
  
  fetchTotalAnomalies = async () => {
    const response = await fetch('http://localhost:8080/api/v1/getTotalAnomalies');
    const body = await response.json();
    this.setState({
      totalAnomalies: body.length
    })
  }
